...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            if args.task_name == 'classification':
                outputs, batch_y_labels = accelerator.gather_for_metrics((outputs, batch_y_labels))
                # outputs shape: (batch_size, num_classes)
                pred = outputs.detach()
                true = batch_y_labels.detach()
                ...
                total_correct += (pred_classes == true).sum().item()
...
def load_content(args):
    ...
    # Check if prompt file exists, if not return default description
    if os.path.exists(prompt_path):
        with open(prompt_path, 'r') as f:
            content = f.read()
        return content
    else:
        # Return default description based on task type
        if args.task_name == 'classification':
            return 'Tor network traffic classification task. Classify network traffic patterns into different categories based on incoming (+1) and outgoing (-1) traffic features.'
        else:
            return 'Time series forecasting task.'